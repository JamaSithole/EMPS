		...
		try {
			if(projtbl != null){		
				...
				if(progtbl != null ){
					...
					if(projtbl.getTotbud() > progtbl.getAvbalance()){
						ValidationBean validationBean = new ValidationBean();
						validationBean.setErrorCode(105);//
						projtbl.setErrorCode(validationBean.getErrorCode());
						validationBean.setErrorMessage("Failed to Add Project, Budget [ " + projtbl.getTotbud() + " ] Allocated greater than Available budget on Program [" + progtbl.getAvbalance() +" ]");
						projtbl.setErrorMessage(validationBean.getErrorMessage());
